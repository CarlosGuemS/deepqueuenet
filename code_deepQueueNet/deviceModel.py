...
class deepQueueNet(trace2Samples):
    ...
    def __init__(self, config, target, data_preprocessing=False):
        ...
        if data_preprocessing:
            print("Build folder start")
            buildfolder(config.modelname)
            print("Build folder end, fet_input start")
            self.fet_input()
            print("fet_input end, _2hdf start")
            self._2hdf()
            print("_2hdf end, _min_max start")
            self._min_max()
            print("_min_max end, _model_input start")
            self.model_input()
            print("_model_input end, _loop_size start")
            self._loop_size()
            print("_loop_size end, merge_sample train start")
            self.merge_sample(task='train')
            print("merge_sample train end, merge_sample test1 start")
            self.merge_sample(task='test1')
            print("merge_sample test1 end, merge_sample test2 start")
            self.merge_sample(task='test2')
            print("merge_sample test2 end")
        else:
            self.load_scaler()
            self._loop_size()
