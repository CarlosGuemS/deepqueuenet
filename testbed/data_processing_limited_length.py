...
def process_sample_total_agg(packet_info_matrix, split, sample_id, max_packets=100000):
    ...
    merged_sequence = [
        ...
        for (ts, size, ii, jj, delay), _ in zip(merged_sequence, range(max_packets))
    ]
    ...
    pd.DataFrame(
        ...
        f"../data/5-port router/full_device_limited/_traces/{split}/5port_sample_{sample_id}.csv",
        index=False,
    )
...
def process_sample_port_agg(
    ...
    for jj, merged_sequence in enumerate(merged_sequences):
        pd.DataFrame(
            ...
            "../data/5-port router/{}_port_limited/_traces/{}/5port_sample{}_port{}.csv".format(
...
# Create folder
shutil.rmtree("../data/5-port router/exit_port_limited", ignore_errors=True)
shutil.rmtree("../data/5-port router/entry_port_limited", ignore_errors=True)
shutil.rmtree("../data/5-port router/full_device_limited", ignore_errors=True)
os.makedirs("../data/5-port router/exit_port_limited/_traces/_train")
os.makedirs("../data/5-port router/exit_port_limited/_traces/_test")
os.makedirs("../data/5-port router/entry_port_limited/_traces/_train")
os.makedirs("../data/5-port router/entry_port_limited/_traces/_test")
os.makedirs("../data/5-port router/full_device_limited/_traces/_train")
os.makedirs("../data/5-port router/full_device_limited/_traces/_test")
